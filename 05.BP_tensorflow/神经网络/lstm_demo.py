...
# init_state = lstm_multi.zero_state(batch_size, dtype=tf.float32)
outputs, final_state = tf.nn.dynamic_rnn(lstm_multi, input_rnn, initial_state=state, time_major=False)
# 当 time_major==False 时， outputs.shape = [batch_size, timestep_size, hidden_size]
# final_state.shape = [layer_num, 2, batch_size, hidden_size] 这里第二个维度2表示，(c,h)两种状态，h为输出，c为传递下一次循环的值
...
with tf.Session() as sess:
    ...
    print(sess.run(final_state[-1][1][0]))
